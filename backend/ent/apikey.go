...
type APIKey struct {
	...
	OrgProjectID *int64 `json:"org_project_id,omitempty"`
...
type APIKeyEdges struct {
	...
	// OrgProject holds the value of the org_project edge.
	OrgProject *OrgProject `json:"org_project,omitempty"`
...
// OrgProjectOrErr returns the OrgProject value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e APIKeyEdges) OrgProjectOrErr() (*OrgProject, error) {
	if e.OrgProject != nil {
		return e.OrgProject, nil
	} else if e.loadedTypes[3] {
		return nil, &NotFoundError{label: orgproject.Label}
	}
	return nil, &NotLoadedError{edge: "org_project"}
}
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldOrgProjectID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field org_project_id", values[i])
			} else if value.Valid {
				_m.OrgProjectID = new(int64)
				*_m.OrgProjectID = value.Int64
			}
		default:
...
// QueryOrgProject queries the "org_project" edge of the APIKey entity.
func (_m *APIKey) QueryOrgProject() *OrgProjectQuery {
	return NewAPIKeyClient(_m.config).QueryOrgProject(_m)
}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	if v := _m.OrgProjectID; v != nil {
		builder.WriteString("org_project_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteByte(')')
