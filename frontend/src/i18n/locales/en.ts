export default {
  ...
  modelPlaza: {
    ...
    pricingStandardDesc: 'You buy universal balance in RMB, and calls are deducted in real time using official API token pricing. USD 1 of official consumption = 1 balance consumed here. The same balance can be used across platforms and groups, but different channels and multipliers change the effective amount of usable USD. Larger packages are usually more cost-effective.',
    ...
    pricingNoticeShared: 'Your topped-up balance can be used across platforms, but different groups have different channels and multipliers, so effective usable USD will vary.',
    ...
    sitePriceCardDesc: 'Each group price is shown after applying the current display multiplier, including cache pricing when available. Unit: site balance / MTok.',
    ...
    tableDiscount: 'Discount / Multiplier',
    ...
    pricingFormula: 'The official column uses USD / MTok, group columns use balance / MTok, and displayed site prices are recalculated from the current group multiplier.',
    pricingAvailableOnly: 'Only groups with pricing are shown',
    pricingCompactHint: 'Cache pricing follows the same multiplier, and unsupported groups no longer take up space.',
    ...
    pricingTableFootnote: 'Pricing data comes from the site’s existing model configuration plus official pricing fields. Displayed group prices change automatically with the configured display multiplier, channel, and model mapping.',
