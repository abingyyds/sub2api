export default {
  ...
  modelPlaza: {
    ...
    pricingStandardDesc: '本站以人民币购买通用余额，调用时按 API 官方 Token 价格实时扣费。官方消费 1 美元 = 本站消耗 1 余额；同一余额可在不同平台与分组通用，但因渠道与倍率不同，折算后的可用美金额度会不同，通常套餐越大越划算。',
    pricingNoticeOfficialTitle: '官方实时扣费',
    pricingNoticeOfficial: '所有调用按对应模型的官方 Token 单价实时结算，不同模型成本不同。',
    pricingNoticeSharedTitle: '余额全站通用',
    pricingNoticeShared: '充值后的余额可跨平台使用，但不同分组的渠道和倍率不同，等效可用美金额度也会不同。',
    ...
    sitePriceCardDesc: '各分组价格按展示倍率换算后展示，缓存价也按相同倍率计算。单位为站内余额 / MTok。',
    ...
    pricingFormula: '官方列单位为 USD / MTok，分组列单位为余额 / MTok；本站展示价按当前分组展示倍率实时换算。',
    pricingAvailableOnly: '仅显示有价格的分组',
    pricingCompactHint: '缓存价按同倍率换算，未支持的分组不再单独占位。',
    ...
    pricingTableFootnote: '价格数据来自本站现有模型配置与官方价格字段；分组展示价格会随展示倍率、渠道和模型映射实时变化。',
